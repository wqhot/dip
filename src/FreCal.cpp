...
namespace dip
{
    ...
    int FreCal::reaFFT(float *src, Complex *dst, int size_n)
    {
        ...
        tic.toc_print_restart("bfFFTComplexRemap");
        FFTComplexRemap(src_com, size_n);
        tic.toc_print_restart("FFTComplexRemap");
        for (int i = 0; i < k; i++)
        {
            z = 0;
            for (int j = 0; j < size_n; j++)
        ...
        tic.toc_print_restart("xunhuan");
        for (int i = 0; i < size_n; i++)
        {
            copyComplex(&src_com[i], &dst[i]);
        }
        tic.toc_print_restart("copyComplex");
        delete[] src_com;
        return 0;
    }
    ...
    Complex *FreCal::DFT2D(Matrix &mat /*, Complex * dst*/)
    {
        ...
        tic.toc_print_restart("expand");
        wn = new Complex[size_w];
        for (int i = 0; i < size_w / 2; i++)
        {
            getW(i, size_w, wn + i);
        }
        tic.toc_print_restart("getW");
        float *src = new float[size_w * size_h];
        Complex *dst = new Complex[size_w * size_h];
        ...
        tic.toc_print_restart("new src");
        Complex *temp = new Complex[size_w * size_h];
        for (int i = 0; i < size_h; i++)
        {
            reaFFT(&src[size_w * i], &temp[size_w * i], size_w);
        }
        tic.toc_print_restart("reaFFT");
        delete[] wn;
        wn = new Complex[size_h];
    ...
    int FreCal::IDFT2D(Complex *src, Matrix &dest, int size_w, int size_h, int old_w, int old_h)
    {
        ...
        Complex *dst = new Complex[size_h * size_w];
        dest.destory();
