...
namespace dip
{
    ...
    Image mirror(Image imageMat, int mode)
    {
        Image dest;
        if (imageMat.channel == 1)
        {
            ...
            if (mode == 0)
            { //水平翻转
                float *temp = new float[dest.matrixB.height];
            ...
            {
                float *temp = new float[dest.matrixB.width];
    ...
    // 图像旋转
    Image rotate(Image imageMat, float degree, float fillcolor, int mode)
    {
        degree = -degree;
            ...
            { //双线性插值
                ...
                for (int i = 0; i < dest.matrixB.height; i++)
                {
                    for (int j = 0; j < dest.matrixB.width; j++)
                    {
                        ...
                        {
                            temp = (int)(imageMat.matrixB.getPixel(inty + 2, intx + 2) * invx * invy + invx * v * imageMat.matrixB.getPixel(inty + 2, intx + 1) + invy * u * imageMat.matrixB.getPixel(inty + 1, intx + 2) + u * v * imageMat.matrixB.getPixel(inty + 1, intx + 1)) >> 22;
                            dest.matrixB.setPixel(i + 1, j + 1, (float)temp);
                            temp = (int)(imageMat.matrixG.getPixel(inty + 2, intx + 2) * invx * invy + invx * v * imageMat.matrixG.getPixel(inty + 2, intx + 1) + invy * u * imageMat.matrixG.getPixel(inty + 1, intx + 2) + u * v * imageMat.matrixG.getPixel(inty + 1, intx + 1)) >> 22;
                            dest.matrixG.setPixel(i + 1, j + 1, (float)temp);
                            temp = (int)(imageMat.matrixR.getPixel(inty + 2, intx + 2) * invx * invy + invx * v * imageMat.matrixR.getPixel(inty + 2, intx + 1) + invy * u * imageMat.matrixR.getPixel(inty + 1, intx + 2) + u * v * imageMat.matrixR.getPixel(inty + 1, intx + 1)) >> 22;
            ...
            else if (mode == 3)
            {
                ...
                for (int i = 0; i < dest.matrixB.height; i++)
                {
                    for (int j = 0; j < dest.matrixB.width; j++)
                    {
                        ...
                        else if (i >= dest.matrixB.height - 3 || j >= dest.matrixB.width - 3)
                        { //右、下边缘
                            int tempi = i - midH;
                            int tempj = j - midW;
                            int newx = (int)(tempj * alpha - tempi * beta + 0.5);
                        ...
                        else
                        {
                            ...
                            if (inti + oldMidW > 1 && inti + oldMidW < imageMat.matrixB.width - 2 && intj + oldMidH >= 1 && intj + oldMidH < imageMat.matrixB.height - 2)
                            {
                                ...
                                A[0] = sinpixdivpix(1 + u);
                                A[1] = sinpixdivpix(u);
                                A[2] = sinpixdivpix(u - 1);
                                A[3] = sinpixdivpix(u - 2);
                                C[0] = sinpixdivpix(1 + v);
                                C[1] = sinpixdivpix(v);
                                C[2] = sinpixdivpix(v - 1);
                                C[3] = sinpixdivpix(v - 2);
                                ...
                                Matrix mB2(4, 4, B2);
                                Matrix mB3(4, 4, B3);
                                Matrix result1 = mA.mul(mB1);
                                result1 = result1.mul(mC);
                                dest.matrixB.setPixel(i + 1, j + 1, result1.getPixel(1, 1));
                                Matrix result2 = mA.mul(mB2);
                                result2 = result2.mul(mC);
                                dest.matrixG.setPixel(i + 1, j + 1, result2.getPixel(1, 1));
                                Matrix result3 = mA.mul(mB3);
                                result3 = result3.mul(mC);
    ...
    // 平移
    Image translation(Image imageMat, int dx, int dy, float fillcolor)
    {
        Image dest;
        dx = -dx;
        dy = -dy;
    ...
    Image cut(Image imageMat, int startrow, int startcol, int endrow, int endcol)
    {
        ...
        if (imageMat.channel == 3)
        {
            dest.channel = 3;
            float *temp;
            temp = new float[endcol - startcol + 1];
    ...
    // 缩放
    Image zoom(Image imageMat, float scalex, float scaley, int mode)
    {
        Image dest;
    ...
    // 滤波
    Image filter(Image imageMat, int type, int size, int T)
    {
        if (size % 2 != 1 || size < 3)
        {
            size = 3;
        ...
        if (imageMat.channel == 3)
        {
            ...
            dest.matrixG = imageMat.matrixG.copy();
            dest.matrixR = imageMat.matrixR.copy();
            float *infof = new float[size * size];
    ...
    // 产生一个滤波窗口
    float getFilterWindow(int type, Matrix info, int size, int T)
    {
        float result = 0.0f;
        if ((size - 1) % 2 != 0)
        {
        ...
        else if (type == 8)
        {
            float temp[9];
            memcpy(temp, info.data, sizeof(float) * 9);
            result = fabs(temp[6] + temp[7] + temp[8] - temp[0] - temp[1] - temp[2]) + fabs(temp[2] + temp[5] + temp[8] - temp[0] - temp[3] - temp[6]);
        }
        else if (type == 9)
        {
            float temp[9];
            memcpy(temp, info.data, sizeof(float) * 9);
            result = fabs(temp[6] + 2 * temp[7] + temp[8] - temp[0] - 2 * temp[1] - temp[2]) + fabs(temp[2] + 2 * temp[5] + temp[8] - temp[0] - 2 * temp[3] - temp[6]);
        }
        else if (type == 10)
        {
            float temp[9];
